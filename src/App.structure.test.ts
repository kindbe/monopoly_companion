...
describe("App file organization", () => {
  ...
  it("defines no React components other than App", () => {
    const componentDefinitions = [...appSource.matchAll(/^(?:export default )?function ([A-Z][A-Za-z0-9]*)\(/gm)].map((match) => match[1]);

    expect(componentDefinitions).toEqual(["App"]);
  });

  it("does not use relative imports", () => {
    const relativeImports = [...appSource.matchAll(/from ["']\./g)].map((match) => match[0]);
