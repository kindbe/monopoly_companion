...
describe("SetupScreen", () => {
  it("wires setup controls to their callbacks", async () => {
    ...
    fireEvent.change(screen.getByLabelText(/bid increment/i), {
      target: { value: "20" }
    })
    fireEvent.change(screen.getByLabelText(/bid deadline/i), {
      target: { value: "99" }
    })
    await user.click(screen.getByLabelText(/include railroads/i))
    await user.click(screen.getByLabelText(/include utilities/i))
    fireEvent.change(screen.getByLabelText(/property count/i), {
      target: { value: "4" }
    })
