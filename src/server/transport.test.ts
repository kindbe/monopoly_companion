...
describe("WebSocket command transport", () => {
  it("handles create, join, start, bid, and skip commands with role-filtered events", () => {
    ...
    handler.handle(
      "host",
      JSON.stringify({ type: "start-bidding", joinCode: "TABLE1" })
    )
    const startedState = findLastEvent(sent.get("host") ?? [], "player-state")
    const openingBid =
      startedState?.type === "player-state" ? startedState.state.currentBid : 0
    handler.handle(
      "host",
      JSON.stringify({
        type: "raise-bid",
    ...
    expect(
      hostEvents.some(
        (event) =>
          event.type === "host-state" && event.state.joinCode === "TABLE1"
      )
    ).toBe(true)
    expect(
      playerEvents.some(
        (event) => event.type === "joined" && event.playerId === "player-1"
      )
    ).toBe(true)
  ...
  it("sends errors for invalid JSON and unknown commands", () => {
    ...
    expect(send).toHaveBeenCalledWith("client-1", {
      type: "error",
  ...
  it("marks joined players disconnected when their client disconnects", () => {
    ...
    handler.handle(
      "host",
      JSON.stringify({ type: "create-session", hostName: "Host" })
    )
    handler.handle(
      "joelle-client",
      JSON.stringify({
