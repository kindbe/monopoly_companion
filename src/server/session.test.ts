...
describe("multiplayer session store", () => {
  ...
  it("creates unique join codes and rejects invalid joins", () => {
    ...
    const store = createSessionStore({
      codeGenerator: () => codes.shift() ?? "TABLE3"
    })
    ...
    expect(() =>
      store.joinSession({ joinCode: "MISSING", name: "Durd" })
    ).toThrow("Session not found.")
    expect(() =>
      store.joinSession({ joinCode: first.joinCode, name: "" })
  ...
  it("creates the host as the first player and rejects blank host names", () => {
    ...
    expect(() => store.createSession({ hostName: " " })).toThrow(
    ...
    expect(store.getHostState(session.joinCode).players).toEqual([
      { id: "player-1", name: "Durd", connected: true }
    ])
    expect(
      store.getPlayerState(session.joinCode, session.playerId).player
    ).toMatchObject({
      id: "player-1",
      name: "Durd",
      remainingCash: 1500,
  ...
  it("defaults countdown to 10 seconds and clamps configured deadlines between 5 and 30 seconds", () => {
    ...
    const defaultSession = store.createSession({
      hostName: "Default Host",
      propertyCount: 1,
      increment: 10
    })
    const shortSession = store.createSession({
      hostName: "Short Host",
      propertyCount: 1,
      increment: 10,
      countdownSeconds: 1
    })
    const longSession = store.createSession({
      hostName: "Long Host",
      propertyCount: 1,
      increment: 10,
      countdownSeconds: 99
    ...
    expect(store.getHostState(defaultSession.joinCode).countdownRemaining).toBe(
      10
    )
  ...
  it("records bids and skips, rejects late bids, and resolves countdown expiry", () => {
    ...
    const isaac = store.joinSession({
      joinCode: session.joinCode,
      name: "Isaac"
    })

    store.startBidding({ joinCode: session.joinCode, now: 1000 })
    const openingBid = store.getPlayerState(
      session.joinCode,
      session.playerId
    ).currentBid
    ...
    store.skipProperty({
      joinCode: session.joinCode,
      playerId: isaac.playerId,
      now: 3000
    })

    expect(
      store.getPlayerState(session.joinCode, session.playerId).currentBid
    ).toBe(openingBid + 10)
  ...
  it("marks skipped players, blocks them from bidding again, and advances when everyone skips", () => {
    ...
    store.startBidding({ joinCode: session.joinCode, now: 1000 })
    const firstProperty = store.getHostState(session.joinCode).currentProperty
      ?.id
    ...
    expect(
      store.getPlayerState(session.joinCode, session.playerId).hasSkipped
    ).toBe(true)
    expect(() =>
      store.raiseBid({
        joinCode: session.joinCode,
    ...
    expect(hostState.completedBids).toEqual([
      expect.objectContaining({ winnerId: null, price: 0 })
    ])
