...
describe("WebRTC signaling handler", () => {
  it("registers hosts, matches players, and relays negotiation messages", () => {
    ...
    handler.handle(
      "host",
      JSON.stringify({ type: "register-host", joinCode: "table1" })
    )
    handler.handle(
      "player",
      JSON.stringify({
  ...
  it("rejects unknown join codes, duplicate hosts, and game-state messages", () => {
    ...
    expect(send).toHaveBeenCalledWith("second-host", {
      type: "signaling-error",
      message: "Join code is already registered."
    })
    expect(send).toHaveBeenCalledWith("player", {
      type: "signaling-error",
      message: "Signaling session not found."
    })
    expect(send).toHaveBeenCalledWith("player", {
      type: "signaling-error",
      message: "Game messages are not allowed on the signaling channel."
    })
  })
  ...
  it("cleans up sessions on disconnect and TTL expiry", () => {
    ...
    handler.handle(
      "host-2",
      JSON.stringify({ type: "register-host", joinCode: "TABLE2" })
    )
    now = 1001
    ...
    expect(send).toHaveBeenCalledWith("player-2", {
      type: "signaling-error",
      message: "Signaling session not found."
    })
  })
