...
describe("WebSocket multiplayer transport", () => {
  it("sends multiplayer commands and forwards server events", () => {
    ...
    transport.connect();
    socket.open();
    transport.createSession({ hostName: "Host", config: { propertyCount: 1, increment: 10 } });
    transport.joinSession({ joinCode: "TABLE1", name: "Joelle" });
    transport.startBidding("TABLE1");
    transport.raiseBid({ joinCode: "TABLE1", playerId: "player-1", increment: 10 });
    transport.skipProperty({ joinCode: "TABLE1", playerId: "player-2" });

    const event: ServerEvent = { type: "joined", joinCode: "TABLE1", playerId: "player-1" };
    socket.message(event);

    expect(WebSocketCtor).toHaveBeenCalledWith("ws://example.test");
    expect(socket.sent.map((message) => JSON.parse(message))).toEqual([
      { type: "create-session", hostName: "Host", config: { propertyCount: 1, increment: 10 } },
      { type: "join-session", joinCode: "TABLE1", name: "Joelle" },
      { type: "start-bidding", joinCode: "TABLE1" },
      { type: "raise-bid", joinCode: "TABLE1", playerId: "player-1", increment: 10 },
...
class FakeWebSocket {
  ...
  message(event: ServerEvent) {
    this.listeners.get("message")?.forEach((listener) => listener({ data: JSON.stringify(event) }));
  }
}
