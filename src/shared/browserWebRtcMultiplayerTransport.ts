...
import {
  createHostAuthoritativeSession,
  type HostAuthoritativeSession
} from "./hostAuthoritativeSession"
import {
  createWebRtcHostTransport,
  createWebRtcPlayerTransport,
  type WebRtcHostTransport,
  type WebRtcPlayerTransport
...
export function createBrowserWebRtcMultiplayerTransport({
  ...
  function connect() {
    ...
    signalingSocket.addEventListener("message", (event) => {
      const signalingEvent = JSON.parse(
        String(event.data)
      ) as SignalingServerEvent
  ...
  function startBidding(joinCode: string) {
    ...
    onEvent({
      type: "player-state",
      state: hostSession.getPlayerState(playerId)
    })
    emitHostState()
  }

  function raiseBid(
    command: Omit<Extract<ClientCommand, { type: "raise-bid" }>, "type">
  ) {
    if (hostSession) {
  ...
  function skipProperty(
    command: Omit<Extract<ClientCommand, { type: "skip-property" }>, "type">
  ) {
    if (hostSession) {
      hostSession.handlePeerIntent("host", {
        type: "skip-property",
        ...command
      })
      if (playerId) {
  ...
  function startHostTimer() {
    ...
    hostTimer = window.setInterval(() => {
      ...
      emitHostState()
      if (
        hostSession.getHostState().phase === "complete" &&
        hostTimer !== null
      ) {
        window.clearInterval(hostTimer)
        hostTimer = null
