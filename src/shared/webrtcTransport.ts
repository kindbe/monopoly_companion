...
type PeerConnectionLike = {
  ...
  onicecandidate?:
    ((event: { candidate: RTCIceCandidateInit | null }) => void) | null
...
export type WebRtcPlayerTransport = {
  ...
  sendJoin(): void
  raiseBid(
    input: Omit<Extract<ClientCommand, { type: "raise-bid" }>, "type">
  ): void
  skipProperty(
    input: Omit<Extract<ClientCommand, { type: "skip-property" }>, "type">
  ): void
}
...
export function createWebRtcHostTransport({
  ...
  const peers = new Map<
    string,
    { connection: PeerConnectionLike; channel: DataChannelLike }
  >()
