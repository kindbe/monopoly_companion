...
describe("shared multiplayer session engine", () => {
  ...
  it("enforces per-property bid limits, tracks current bidder, and keeps the deadline fixed", () => {
    ...
    const joelle = engine.joinSession({ joinCode: host.joinCode, name: "Joelle" });

    engine.startBidding({ joinCode: host.joinCode, now: 1_000 });
    const openingBid = engine.getHostState(host.joinCode).currentBid;

    expect(engine.getPlayerState(host.joinCode, host.playerId).remainingBidCount).toBe(3);
    expect(engine.getHostState(host.joinCode).currentBidderName).toBeNull();

    engine.raiseBid({ joinCode: host.joinCode, playerId: host.playerId, increment: 10, now: 2_000 });
    engine.raiseBid({ joinCode: host.joinCode, playerId: joelle.playerId, increment: 10, now: 3_000 });
    engine.raiseBid({ joinCode: host.joinCode, playerId: host.playerId, increment: 10, now: 4_000 });
    engine.raiseBid({ joinCode: host.joinCode, playerId: host.playerId, increment: 10, now: 5_000 });
    vi.setSystemTime(5_000);
    ...
    expect(engine.getPlayerState(host.joinCode, host.playerId).remainingBidCount).toBe(0);
    expect(engine.getPlayerState(host.joinCode, joelle.playerId).remainingBidCount).toBe(2);
    expect(() => engine.raiseBid({ joinCode: host.joinCode, playerId: host.playerId, increment: 10, now: 6_000 })).toThrow(
      "No bids remaining for this property."
    );
  ...
  it("defaults max bids to three and clamps configured max bids between one and five", () => {
    ...
    const engine = createSessionEngine({ codeGenerator: () => codes.shift() ?? "TABLE4", random: () => 0.1 });
    const defaultSession = engine.createSession({ hostName: "Default", propertyCount: 1, increment: 10 });
    const shortSession = engine.createSession({ hostName: "Short", propertyCount: 1, increment: 10, maxBidsPerPlayer: 0 });
    const longSession = engine.createSession({ hostName: "Long", propertyCount: 1, increment: 10, maxBidsPerPlayer: 99 });
    ...
    expect(engine.getPlayerState(defaultSession.joinCode, defaultSession.playerId).remainingBidCount).toBe(3);
    expect(engine.getPlayerState(shortSession.joinCode, shortSession.playerId).remainingBidCount).toBe(1);
    expect(engine.getPlayerState(longSession.joinCode, longSession.playerId).remainingBidCount).toBe(5);
  });

  it("does not let a skip erase an already submitted bid", () => {
    const engine = createSessionEngine({ codeGenerator: () => "TABLE1", random: () => 0.1 });
    const host = engine.createSession({ hostName: "Host", propertyCount: 1, increment: 10 });
    ...
    engine.raiseBid({ joinCode: host.joinCode, playerId: host.playerId, increment: 10, now: 2_000 });
    engine.skipProperty({ joinCode: host.joinCode, playerId: host.playerId, now: 3_000 });
