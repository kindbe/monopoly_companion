...
describe("WebRTC multiplayer transports", () => {
  it("negotiates host and player data channels and sends game messages directly", async () => {
    ...
    const host = createWebRtcHostTransport({
      ...
      sendSignal: (message) =>
        signalingMessages.push({ from: "host", message }),
    ...
    const player = createWebRtcPlayerTransport({
      ...
      RTCPeerConnectionCtor: fakePeerConnectionFactory(peers),
      sendSignal: (message) =>
        signalingMessages.push({ from: "player", message }),
    ...
    expect(hostIntents).toEqual([
      {
        peerId: "player-peer",
        message: { type: "join-session", joinCode: "TABLE1", name: "Joelle" }
      },
      {
        peerId: "player-peer",
        message: {
    ...
    expect(playerEvents).toEqual([
      { type: "joined", joinCode: "TABLE1", playerId: "player-2" }
    ])
  })
...
function linkPeers(
  hostPeer: FakePeerConnection,
  playerPeer: FakePeerConnection
) {
