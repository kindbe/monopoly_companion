...
describe("host-authoritative multiplayer session", () => {
  it("owns session state in the host, routes peer intents, broadcasts private snapshots, and resolves countdowns", () => {
    ...
    session.handlePeerIntent("joelle-peer", { type: "join-session", joinCode: "TABLE1", name: "Joelle" });
    session.startBidding(1000);
    const openingBid = session.getHostState().currentBid;
    session.handlePeerIntent("joelle-peer", { type: "raise-bid", joinCode: "TABLE1", playerId: "player-2", increment: 10 });
    ...
    expect(finalHostState.completedBids).toEqual([expect.objectContaining({ winnerId: "player-2", price: openingBid + 10 })]);
    expect(sent).toEqual(
      expect.arrayContaining([
        { peerId: "joelle-peer", event: { type: "joined", joinCode: "TABLE1", playerId: "player-2" } },
        expect.objectContaining({
          ...
          event: expect.objectContaining({
            ...
            state: expect.objectContaining({
              role: "player",
              player: expect.objectContaining({ id: "player-2", name: "Joelle" })
            })
          })
        })
      ])
    );
    expect(JSON.stringify(sent.find((event) => event.peerId === "joelle-peer"))).not.toContain("Host");
  ...
  it("marks disconnected peers unavailable and restores their latest snapshot on reconnect", () => {
    ...
    expect(session.getHostState().players).toContainEqual({ id: "player-2", name: "Joelle", connected: false });

    session.reconnectPeer("joelle-peer-2", "player-2");

    expect(session.getHostState().players).toContainEqual({ id: "player-2", name: "Joelle", connected: true });
    expect(sent).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
