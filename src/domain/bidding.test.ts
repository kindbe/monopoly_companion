...
describe("property card data", () => {
  it("includes street deed stats for Boardwalk", () => {
    const boardwalk = MONOPOLY_PROPERTIES.find((property) => property.id === "boardwalk");
  ...
  it("includes railroad and utility deed stats", () => {
    expect(MONOPOLY_PROPERTIES.find((property) => property.id === "reading-railroad")).toMatchObject({
      category: "railroad",
      retailValue: 200,
      mortgage: 100,
      rentDescription: expect.stringContaining("$25")
    });
    expect(MONOPOLY_PROPERTIES.find((property) => property.id === "electric-company")).toMatchObject({
      category: "utility",
      retailValue: 150,
      mortgage: 75,
  ...
  it("calculates opening bids as 25% of retail value rounded up to the nearest $10", () => {
    const findProperty = (id: string) => {
      const property = MONOPOLY_PROPERTIES.find((candidate) => candidate.id === id);
...
describe("ascending auction", () => {
  ...
  it("starts at a property opening bid and accepts quick increment raises", () => {
    ...
    expect(() => placeAscendingBid(auction, "player-2", 220, 200)).toThrow("Bid cannot exceed remaining cash.");
  });
