...
- **THEN** the system SHALL create a host session, join two players by code, start bidding, submit one bid, submit one skip, resolve the property, and verify session completion
...
- **THEN** the system SHALL allow the session countdown to be shorter than the production 30-second countdown
